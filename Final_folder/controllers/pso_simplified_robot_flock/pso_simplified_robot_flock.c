...
#define DATASIZE 5                    // Number of elements in particle (2 Neurons with 8 proximity sensors and 5 params for flocking)
...
static measurement_t _meas; // See class in utils
static double last_gps_time_s = 0.0f;

// Initial robot positions (x, theta, y) --> in pose (x,y,heading)
pose_t _pose_origin_robot_0 = {0, 0.0, 0};
pose_t _pose_origin_robot_1 = {0, 0.1, 0};
pose_t _pose_origin_robot_2 = {0, -0.1, 0};
pose_t _pose_origin_robot_3 = {0, 0.2, 0};
pose_t _pose_origin_robot_4 = {0, -0.2, 0};
...
void reynolds_rules(double rule1_threshold, double rule1_weight, double rule2_threshold, double rule2_weight, double migration_weight) {
   ...
   float cohesion[2] = {0,0};       // Reynold's rule for flocking
   float dispersion[2] = {0,0};     // Reynold's rule for flocking
...
double simulation_webot(double weights[DATASIZE+1]){
  ...
  for(j=0;j<SIM_STEPS;j++){
    ...
    reynolds_rules( weights[0]/SCALING_REYNOLD, weights[1]/SCALING_REYNOLD, weights[2]/SCALING_REYNOLD, weights[3]/SCALING_REYNOLD,  weights[4]/SCALING_REYNOLD);
